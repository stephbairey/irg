...
## D072 — Gaggle "little themes" are child themes of The Bulletin Local (plugin 3.20.0)

- **Status**: Decided
- **Date**: 2026-08-31
- **Context**: Post-launch migrations of calgary, montreal, and western-mass (WXR imports done 2026-08-31, media pulled from the still-live pinned old sites). Maya wants each gaggle's subsite to "essentially match their old site" visually while migrating content in near-existing form, with Astro-rendered gaggle pages as a later phase (hybrid decision).
- **Options considered**: standalone WP theme per gaggle (duplicates the seeder, Gaggle Settings, contact endpoint, song templates ~20 files each); Elementor network-wide to render imported builder pages as-is (heavy, granny-hostile, against the headless direction); child themes of the-bulletin-local.
- **Choice**: one child theme per gaggle (`wp-theme/<slug>-local/`): a `style.css` with `Template: the-bulletin-local`, a `functions.php` that enqueues an `overrides.css` after the parent stylesheet, and the override file swapping the parent's CSS custom properties (palette, fonts, shadows) plus a few small rules. Parent templates, seeder, and Gaggle Settings all inherit. `irg-core` 3.20.0 generalizes `/wp-json/irg/v1/theme-upload` to accept `the-bulletin-local.zip` or any `<slug>-local.zip` (sanitize_key-validated); `deploy-theme.mjs` takes the slug as an argument. Shipped: western-mass-local (GeneratePress look: white/Arial/blue links), calgary-local (white/Arial Black/#bd081c red), montreal-local (Open Sans/mustard #f4be68/plum #804872).
- **Trade-off**: old sites' layouts are echoed, not cloned — the Bulletin Local structure (hard-coded nav: about/actions/photos/videos/contact) stays, so imported legacy pages must be mapped into the standard pages editorially. Montreal's Elementor page content renders as flattened markup until hand-converted to blocks.
- **Revisit if**: a gaggle needs structural (not just visual) divergence — then the child theme grows template overrides; or when the Astro per-gaggle phase lands and these become per-gaggle tokens in the frontend instead.

## D073 — Per-song central visibility list for hidden gaggles (Seattle relaunch)

- **Status**: Decided
- **Date**: 2026-09-01
- **Context**: The Seattle song librarian delivered a per-song list (xlsx): 208 of 448 Seattle songs marked to display on raginggrannies.org. D052 hides all Seattle songs; D069's `feature_on_homepage` escape hatch doubles as homepage placement, so flagging 208 songs would swamp the committee's curated 21-song homepage pool. This is exactly D069's stated revisit condition (public-without-featuring).
- **Options considered**: a second WP field (`display_centrally`) with plugin/GraphQL/snapshot plumbing; abolishing the hidden-gaggle rule for Seattle and hiding the 240 individually; a committed per-song overrides file.
...
## D075 — Every gaggle subsite lives at `<slug>.raginggrannies.org` (wildcard mapping)

- **Status**: Decided
- **Date**: 2026-09-04 (applied), 2026-09-08 (directory links pushed)
- **Context**: Five migrated gaggles already had mapped subdomains (D072 era); the other 56 sat at `cms.raginggrannies.international/<slug>/`, which leaked the CMS hostname into find-a-gaggle links, the How To guides, and every login URL a granny would be told. Maya asked for one consistent address shape.
- **Choice**: a single wildcard: cPanel `*.raginggrannies.org` subdomain (the wildcard form is disabled on this host; created via `uapi SubDomain addsubdomain` after adding the apex as an addon domain), a Cloudflare Origin CA wildcard certificate installed on that vhost (AutoSSL cannot issue wildcards; key and CSR generated on the server), and a proxied `*` A record in Cloudflare. A server-side sweep flipped all 56 `irg_blogs` rows to `domain=<slug>.raginggrannies.org, path=/` and set `home`/`siteurl`/`irg_slug`. Explicit DNS records still win over the wildcard, so a gaggle that runs its own site elsewhere keeps its record and gets a `website` override in `data/gaggle-locations.json` (Rochester today); its subsite stays on a cms path so the snapshot script can reach it.
- **Trade-off**: adding the apex to cPanel made it think it hosts mail for raginggrannies.org; fixed by pointing the cPanel-local MX at Cloudflare so detection reads "remote". Old `cms.../<slug>/` URLs now 404 (nothing public linked them). Anything deriving gaggle identity from the blog path must use `irg_slug` (the third path-slash bug, theme 1.27.2, is the precedent).
- **Revisit if**: a gaggle with an external site wants to move onto the multisite (delete its DNS record, drop the override, map the blog), or the Origin CA certificate approaches expiry (2041).

## D076 — Allow-listed songwriters publish immediately from /submit/ (plugin 3.23.0)

- **Status**: Decided
- **Date**: 2026-09-09
- **Context**: Since launch, every song from the Submit page lands as a draft for the librarian to approve (workstream A hardening). Lee Stanfield, who publishes more songs than almost anyone and posts each song's link to the E-Vine the same day, objected that the queue breaks a list-wide agreement to share songs only as archive links. Maya's reply (2026-09-08) promised that songs submitted under Lee's name would be approved automatically while the review step stays for everyone else.
- **Choice**: a per-songwriter allow-list, not a global switch. Main-site option `irg_autopublish_songwriters` (JSON array of names, WP-CLI managed for now) is checked in the submit endpoint; if any name in the comma-separated songwriter field matches, case- and whitespace-insensitively, the post is inserted as `publish` instead of `draft`. The librarian still gets the notification, worded as "published" with an edit link; the endpoint response carries `published` and the permalink. Seeded with Lee Stanfield.
- **Trade-off**: the match is on a free-text name, so anyone who knows the submit password and types "Lee Stanfield" publishes instantly. Acceptable: the password already gates the form, the librarian is notified either way, and unpublishing is one click. Publish is still not instant on the public site; the twice-daily snapshot refresh sets the ceiling until workstream C lands.
- **Revisit if**: the librarian wants a settings screen (move the option to a field on the Songs admin page), the list grows past a handful of names, or workstream C makes the wait short enough that the queue stops mattering.

## D077 — Song saves trigger the content refresh (workstream C, plugin 3.24.0)

- **Status**: Decided
- **Date**: 2026-09-10
- **Context**: Since cutover the public site has read songs from a committed JSON snapshot that a GitHub Actions workflow regenerates every 12 hours. Grannies editing songs read the wait as failure (Rieko, 2026-09-09). Submissions are rare: Maya estimates a very heavy day at five songs and most days at zero. The plan's original workstream C (PHP exporter, Cloudflare Worker, HMAC, GitHub contents API) was designed before the refresh workflow existed and would duplicate it.
- **Choice**: WP asks GitHub to run the existing `refresh-content.yml` workflow via `workflow_dispatch`, ten minutes after the last qualifying song change. Qualifying: any save of a published song (admin, `/edit-song/`, bulk edit, allow-listed submit), a song leaving publish, permanent deletion, and the ACF `display_centrally` toggle. Debounce is one re-armed WP-Cron single event; a host cron runs due events every five minutes so the delay is bounded without CMS traffic. Token is a fine-grained PAT (Actions: write, this repo only) in `wp-config.php` as `IRG_GITHUB_TOKEN`. Dispatch failure logs and emails Maya; the granny's save always succeeds. The 12-hour schedule stays as the backstop.
